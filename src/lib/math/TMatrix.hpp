...
#if MATRIX_EXCEPTION
#define  THROW_MATRIX_EXECPTION throw(class_exception<TMatrix>)
#define ASSERT_MATRIX_EXCEPTION(c,m) if(!c){  throw class_exception<TMatrix>(m);}
#else
        ...
        */
        ...
        f_inline TMatrix<DATATYPE, COLUMN, ROW> &inv () THROW_MATRIX_EXECPTION
        {
            ...
            for (k = 0; k < ROW; k++)
            {
                ...
                if ( indx == -1 )
                {
                    return *this;    // can to get the result
                }
        ...
        f_inline TMatrix<DATATYPE, COLUMN, EQUS> solve (const TMatrix<DATATYPE, COLUMN, EQUS> &v) const THROW_MATRIX_EXECPTION
        {
            ...
            for (k = 0; k < ROW; k++)
            {
                ...
                if ( indx == -1 )
                {
                    return s;    // can to get the result
                }
...
/** inversed matrix */
template <typename DATATYPE, size_t ROW, size_t COLUMN>
f_inline seumath::TMatrix<DATATYPE, COLUMN, ROW> operator ! ( const seumath::TMatrix<DATATYPE, ROW, COLUMN> &m )
{
    seumath::TMatrix<DATATYPE, COLUMN, ROW> temp(m);
    return temp.inv();
}

/** tarnsposed matrix */
template <typename DATATYPE, size_t ROW, size_t COLUMN>
f_inline seumath::TMatrix<DATATYPE, ROW, COLUMN> operator ~ ( const seumath::TMatrix<DATATYPE, ROW, COLUMN> &m )
{
    seumath::TMatrix<DATATYPE, ROW, COLUMN> temp(m);
    return temp.transpose();
}

template <typename DATATYPE, size_t ROW, size_t COLUMN>
f_inline seumath::TMatrix<DATATYPE, ROW, COLUMN> operator*(DATATYPE scale, seumath::TMatrix<DATATYPE, ROW, COLUMN> &m)
{
    return (m * scale);
}
...
template <typename DATATYPE, size_t ROW, size_t COLUMN>
f_inline std::istream &operator >> (std::istream &is, seumath::TMatrix<DATATYPE, ROW, COLUMN> &m)
{
    for (size_t i = 0; i < ROW; i++)
        for (size_t j = 0; j < COLUMN; j++)
        {
...
// dump
template <typename DATATYPE, size_t ROW, size_t COLUMN>
f_inline std::ostream &operator<<(std::ostream &os, const seumath::TMatrix<DATATYPE, ROW, COLUMN> &m)
{
    for ( size_t i = 0; i < ROW; i++ )
    {
        for ( size_t j = 0; j < COLUMN; j++ )
